-- Drop all existing policies on requisitions table and recreate them properly

-- Drop all existing policies first
DROP POLICY IF EXISTS "Staff can view own requisitions" ON public.requisitions;
DROP POLICY IF EXISTS "Staff can create own requisitions" ON public.requisitions;
DROP POLICY IF EXISTS "Staff can update own pending requisitions" ON public.requisitions;
DROP POLICY IF EXISTS "Authorizers can view relevant requisitions" ON public.requisitions;
DROP POLICY IF EXISTS "Authorizers can authorize requisitions" ON public.requisitions;
DROP POLICY IF EXISTS "Approvers can view relevant requisitions" ON public.requisitions;
DROP POLICY IF EXISTS "Approvers can approve requisitions" ON public.requisitions;
...
-- Enable RLS on the requisitions table (if not already enabled)
...
-- Policy for authorizers to view all requisitions (they need to see all to process)
CREATE POLICY "Authorizers can view all requisitions" 
ON public.requisitions 
FOR SELECT 
USING (
...
-- Policy for approvers to view all requisitions (they need to see authorized ones)
CREATE POLICY "Approvers can view all requisitions" 
ON public.requisitions 
FOR SELECT 
USING (
